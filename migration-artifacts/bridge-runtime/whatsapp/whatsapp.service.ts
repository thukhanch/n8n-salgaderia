...
interface N8nBridgeInboundPayload {
  sessionId: string;
  sessionName: string;
  from: string;
  pushName: string;
...
export class WhatsappService implements OnModuleDestroy, OnModuleInit {
  ...
  private readonly n8nBridgeWebhookUrl = (process.env.N8N_BRIDGE_WEBHOOK_URL || '').trim();
  private readonly n8nBridgeWebhookToken = (process.env.N8N_BRIDGE_WEBHOOK_TOKEN || '').trim();
  ...
  async onModuleInit() {
    const persistedSessions = await this.sessionRepo.find({
      where: [
    ...
    for (const session of persistedSessions) {
      try {
        await this.connectSession(session.id, () => undefined);
        this.logger.log(`Persisted session ${session.id} reconnected on startup`);
      } catch (error: any) {
        this.logger.error(`Failed to reconnect persisted session ${session.id}: ${error?.message || error}`);
      }
    }
  }

  private extractMessageText(message?: proto.IMessage | null): string {
    if (!message) return '';
    return (
  ...
  private buildN8nBridgeInboundPayload(session: WhatsappSession, msg: proto.IWebMessageInfo): N8nBridgeInboundPayload | null {
    const from = (msg.key?.remoteJid || '').replace('@s.whatsapp.net', '').replace('@c.us', '');
    const text = this.extractMessageText(msg.message);
    if (!from || !text || msg.key?.fromMe) return null;
    return {
      ...
      timestamp: typeof msg.messageTimestamp === 'number' ? msg.messageTimestamp : Number(msg.messageTimestamp || 0) || null,
      fromMe: Boolean(msg.key?.fromMe),
      raw: msg,
    };
  }

  private async forwardInboundToN8n(payload: N8nBridgeInboundPayload): Promise<void> {
    if (!this.n8nBridgeWebhookUrl) return;
    try {
      const response = await fetch(this.n8nBridgeWebhookUrl, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          ...(this.n8nBridgeWebhookToken ? { Authorization: `Bearer ${this.n8nBridgeWebhookToken}` } : {}),
        },
        body: JSON.stringify(payload),
      });
      if (!response.ok) {
        this.logger.warn(`N8N bridge webhook returned ${response.status} ${response.statusText}`);
      }
    } catch (error: any) {
      this.logger.error(`Failed to forward inbound to N8N bridge: ${error?.message || error}`);
    }
  }
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        if (!msg.key.fromMe) {
          ...
          const inboundPayload = this.buildN8nBridgeInboundPayload(session, msg);
          if (inboundPayload) {
            await this.forwardInboundToN8n(inboundPayload);
          }
        }
      }
    });
